...
-- Data validation applies regardless of which client reaches PostgREST. The
-- constraints are NOT VALID so a legacy invalid row cannot block this security
-- migration; PostgreSQL still enforces them for every new or changed row.
alter table public.profiles
  alter column full_name set not null,
  alter column full_name set default 'Student';
alter table public.profiles
  drop constraint if exists profiles_full_name_length,
  add constraint profiles_full_name_length check (char_length(trim(full_name)) between 1 and 160) not valid,
  drop constraint if exists profiles_bio_length,
  add constraint profiles_bio_length check (bio is null or char_length(bio) <= 4000) not valid,
  drop constraint if exists profiles_avatar_url_protocol,
  add constraint profiles_avatar_url_protocol check (avatar_url = '' or avatar_url ~ '^https://') not valid;
...
alter table public.posts
  ...
  add constraint posts_content_length check (char_length(trim(content)) between 1 and 20000) not valid,
  drop constraint if exists posts_tags_limit,
  add constraint posts_tags_limit check (cardinality(tags) <= 10) not valid;

alter table public.ai_notes
  drop constraint if exists ai_notes_content_length,
  add constraint ai_notes_content_length check (char_length(trim(content)) between 1 and 50000) not valid,
  drop constraint if exists ai_notes_summary_length,
  add constraint ai_notes_summary_length check (char_length(trim(summary)) between 1 and 10000) not valid;
